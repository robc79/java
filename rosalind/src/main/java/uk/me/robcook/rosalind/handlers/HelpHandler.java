...
public class HelpHandler extends CommandHandler<HelpCommand>
{
    ...
    {
        var rawArgs = command.getArgs();
        var commandName = command.getParsedArgs().commandName();

        var targetCommand = switch (commandName)
        {
            case CommandName.parse -> new ParseCommand(rawArgs);
            case CommandName.count -> new CountCommand(rawArgs);
            case CommandName.transcribeDna -> new TranscribeDnaCommand(rawArgs);
            default -> new HelpCommand(rawArgs);
        };
